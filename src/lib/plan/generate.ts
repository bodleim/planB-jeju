import { WALK_LIMIT_KM, estimateTravelMinutes, haversineKm, travelModeFor } from '../geo';
import { intervalsAt } from '../hours';
import { createRng, pickIndex } from '../rng';
import { categoryFitOf, scoreVisit, type ScoreContext } from './score';
import type { Exposure, LatLng, MinuteOfDay, Place, TripCategory, Weekday } from '../types';
import type {
  FeasibleVisit,
  Plan,
  PlanInput,
...
export function tryVisit(place: Place, context: VisitContext): VisitAttempt {
  ...
  if (categoryFitOf(place, context.category) <= 0) return { ok: false, reason: 'category_mismatch' };
