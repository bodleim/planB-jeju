import type { TravelMode } from '../geo.ts';
import type {
  LatLng,
  MinuteOfDay,
  Place,
...
export interface PlanPolicy {
  ...
  /**
   * 편도 이동이 남은 시간의 이 비율을 넘으면 후보에서 제외한다 (F3의 거리 제약).
   * `tryVisit`의 past_deadline만으로는 '5시간 남았는데 2시간 거리'가 통과해 버린다.
   */
  readonly maxTravelShare: number;
