...
| 장소·운영정보 (한국관광공사 TourAPI) | **제주 전역 1,010곳**을 사전 수집한 JSON 스냅샷을 저장소에 커밋 (운영정보가 확인된 718곳만 자동 편성). 수집 시각을 화면에 표시 |
...
- 후보는 **제주 전역 1,010곳** 중 운영정보가 확인된 곳만 씁니다. 상세 정보를 아직 못 받은
  일부(수십 곳)는 `python3 scripts/snapshot.py tour-jeju` 재실행으로 이어받을 수 있습니다
  (수집기는 재개 가능).
- 위치는 브라우저 감지 또는 장소 이름 검색으로만 받습니다. 둘 다 없으면 계획을 만들지 않고
  위치를 요청합니다.
